...
bool Console::ReadParameter(const string & line, int & parameter, OptimizingPhase * op_ptr)
{
  ...
  if (iss.fail())
  {
    QString message("Nav iespējams nolasīt parametra vērtību no rindas: ");
    message += QString(line.c_str());
    message += ".\nLūdzu, izlabojiet faila saturu.";
    QMessageBox::critical(op_ptr, QObject::tr("Kļūda"), message);
    return false;
  }
  else
    return true;
...
bool Console::ReadParameter(const string & line, double & parameter, OptimizingPhase * op_ptr)
{
  istringstream iss(line);
  string temp;
  iss >> temp;  // write paramater name
...
bool Console::ReadParameter(const string & line, bool & parameter, OptimizingPhase * op_ptr)
{
  istringstream iss(line);
  string temp;
  iss >> temp;    // write paramater name
...
void Console::PrintMessage(const QString & message)
{
  QMessageBox::information(0, QString("Ziņojums"), message);
}

void Console::PrintErrorMessage(const QString & title, const QString & message)
{
